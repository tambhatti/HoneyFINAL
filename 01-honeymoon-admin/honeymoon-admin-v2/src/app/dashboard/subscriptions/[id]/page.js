...
export default function EditSubscriptionPage({ params }) {
  ...
  useEffect(() => {
    if (plan.id) {
      setForm({
        ...
        commissionDiscount: String(plan.commissionDiscount || ''),
        profileFeaturing:   plan.profileFeaturing  || false,
        bannerPromotion:    plan.bannerPromotion    || false,
  ...
  const handleSave = async () => {
    ...
    try {
      await AdminService.updateSubscriptionPlan(planId, {
        name:               form.name,
        priceMonthly:       Number(form.priceMonthly),
        priceYearly:        Number(form.priceYearly) || undefined,
  ...
  return (
    <div className="w-full min-w-0 max-w-2xl">
      {confirm && <ConfirmModal message="Save changes to this subscription plan?" onYes={() => { setConfirm(false); handleSave(); }} onNo={() => setConfirm(false)} />}
      ...
      <div className="bg-white rounded-2xl p-4 sm:p-8 shadow-[0_0_20px_rgba(0,0,0,0.05)]">
        <h1 className="font-baskerville text-xl sm:text-2xl text-[#1a1a1a] mb-6">
          {loading ? 'Loading…' : `Edit — ${plan.name || 'Subscription Plan'}`}
        </h1>

        {/* Name + Status */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
          <div><label className={lbl}>Plan Name<span className="text-red-500">*</span></label>
            <input value={form.name} onChange={e => f('name', e.target.value)} className={inp}/></div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
          <div><label className={lbl}>Monthly Price (AED)<span className="text-red-500">*</span></label>
            <input type="number" value={form.priceMonthly} onChange={e => f('priceMonthly', e.target.value)} className={inp}/></div>
          <div><label className={lbl}>Yearly Price (AED)</label>
            <input type="number" value={form.priceYearly} onChange={e => f('priceYearly', e.target.value)} className={inp}/></div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
          <div>
            ...
            <p className="text-gray-400 text-xs mt-1">Discount applied to platform commission rate</p>
          </div>
          <div><label className={lbl}>Max Services</label>
            <input type="number" value={form.maxServices} onChange={e => f('maxServices', e.target.value)} className={inp}/></div>
        </div>

        {/* Profile Featuring + Banner Promotion */}
          ...
          <div className="flex flex-col gap-3">
            <label className="flex items-start gap-3 cursor-pointer">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400">Vendor profile shown in featured section</p>
              </div>
            </label>
            <label className="flex items-start gap-3 cursor-pointer">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400">Promotional banner on the home page</p>
              </div>
            </label>
          </div>
        </div>
        ...
        <div className="mb-6">
          ...
          <textarea value={form.features} onChange={e => f('features', e.target.value)} rows={4} className={`${inp} resize-none`}/>
        </div>
